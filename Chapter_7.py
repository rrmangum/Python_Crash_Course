...
# It takes one arguement: the prompt or instructions that we want to display to the user
...
# Must use int() function to treat the input as a numerical value and not a string
...
if rental_car == "subaru":
    ...
    print("I'm sorry, we don't have any Jeeps available. They rent out so fast.")
...
user_number = input("Enter a number and I'll tell you if its a multiple of 10 or not: ")
...
# += is telling Python prompt = prompt + "\nEnter 'quit' to end the program." its just a shorthand version
...
# a flag acts as signal to the program. Used in a program that should run only as long as many conditions are true, 
# you can define one variable that determines whether or not the entire program is active
# write programs so they run while the flag is set to True and stop running when any of the several events sets the 
# value of the flag to False
# this while statement only needs to check one condition: whether or not the flag is currently True
...
while active: # loop continues running as long as the flag remains True
    ...
    if message == 'quit': # if user enters 'quit' change the flag to False and the loop stops
        active = False
    else: # if user enters anything other than 'quit' print their input as a message
        print(message)

# this program is the same output as the previous exercise, but the program is more organized and will be much easier to 
# add future conditions in which the program should end

# the break statement will exit a while loop immediately without running any remaining code 
...
# you can use the break statement in any Python loop, even in an for loop working through a list or dicitionary

# use the continue statement to return to the beginning of the loop based on the result of a conditional test
current_number = 0
while current_number < 10:
    current_number += 1
    if current_number % 2 == 0: # if statement checks the modulo of current_number and 2, if the modulo is 0, the continue statement tells Python to ignore the rest of the loop and return to the beginning
        continue
    print(current_number)
...
while unconfirmed_users: # loop runs as long as the list of unconfirmed_users is not empty
    current_user = unconfirmed_users.pop() # removes unverified users one at a time from the end of unconfirmed_users

    print(f"Verifying user: {current_user.title()}")
    confirmed_users.append(current_user) # add users to the confirmed_users list
...
sandwich_orders = ['BLT', 'Pastrami', 'Tuna Melt', 'Grilled Cheese', 'Pastrami', 'Club', 'Italian', 'Pastrami']
